...
void my_estimateWjets()
{
    ...
    wjetsSS_template[5] = (TH1D*)f->Get("histSameWJets1_Data")->Clone(); //from Data
    ...
    wjets_ratio[0] = (TH1D*)f[0]->Get("histWJets2_DY")->Clone(); //from DY
    wjets_ratio[0]->SetFillColor(2);
    wjets_ratio[0]->SetStats(kFALSE);
    wjets_ratio[0]->Sumw2();

    wjets_ratio[1] = (TH1D*)f[1]->Get("histWJets2_ttbar")->Clone(); //from ttbar
    wjets_ratio[1]->SetFillColor(3);
    wjets_ratio[1]->SetStats(kFALSE);
    wjets_ratio[1]->Sumw2();

    // same sign Wjet (ratio)
    wjetsSS_ratio[5] = (TH1D*)f->Get("histSameWJets2_Data")->Clone(); //from Data
    ...
    // -- Wjet template from same sign Wjet -- //
    ...
    wjets_template[5]->Scale(norm1[2]); //estimated same sign Wjet
    ...
    wjets_ratio[5]->Scale(norm2[2]); //estimated same sign Wjet
