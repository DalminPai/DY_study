...
Double_t ptbin_endcap[ptbinnum_endcap+1] = {52,60,70,80,90,100,150,200,500};
const int ptbinnum = 16;
Double_t ptbin[ptbinnum+1] = {52,60,70,80,90,100,120,140,160,180,200,250,300,350,400,450,500};

const int etabinnum = 2;
Double_t etabin[etabinnum+1] = {0,1.2,2.4};
...
void my_estimateFR()
{
	...
	{
		TH1D* h[8];
		h[0] = (TH1D*)f->Get("denominator_pt_barrel_"+types[i])->Clone("denominator_pt_fit_barrel_"+types[i]);
		h[1] = (TH1D*)f->Get("denominator_pt_barrel_"+types[i])->Clone("denominator_pt_xsec_barrel_"+types[i]);
		h[2] = (TH1D*)f->Get("denominator_barrel_"+types[i])->Clone("denominator_barrel_"+types[i]);
		h[3] = (TH1D*)f->Get("numerator_pt_barrel_"+types[i])->Clone("numerator_pt_barrel_"+types[i]);
		h[4] = (TH1D*)f->Get("denominator_pt_endcap_"+types[i])->Clone("denominator_pt_fit_endcap_"+types[i]);
		h[5] = (TH1D*)f->Get("denominator_pt_endcap_"+types[i])->Clone("denominator_pt_xsec_endcap_"+types[i]);
		h[6] = (TH1D*)f->Get("denominator_endcap_"+types[i])->Clone("denominator_endcap_"+types[i]);
		h[7] = (TH1D*)f->Get("numerator_pt_endcap_"+types[i])->Clone("numerator_pt_endcap_"+types[i]);
		...
		for(int j=0; j<8; j++)
		{
			...
			if( types[i] == "Data" )
				...
				setMCHist( h[j], icolor[i] );

			// Rebin
			TString hName = h[j]->GetName();
			if( hName.Contains("pt") )
			{
				if( hName.Contains("endcap") )
					h[j] = (TH1D*)(h[j]->Rebin(ptbinnum_endcap, "", ptbin_endcap));
				else
					h[j] = (TH1D*)(h[j]->Rebin(ptbinnum, "", ptbin));
			}
			else h[j]->Rebin(2);
		}

		denominator_pt_fit_barrel[i] = (TH1D*)h[0]->Clone();
		denominator_pt_xsec_barrel[i] = (TH1D*)h[1]->Clone();
		denominator_barrel[i] = (TH1D*)h[2]->Clone();
		numerator_pt_barrel[i] = (TH1D*)h[3]->Clone();
		denominator_pt_fit_endcap[i] = (TH1D*)h[4]->Clone();
		denominator_pt_xsec_endcap[i] = (TH1D*)h[5]->Clone();
		denominator_endcap[i] = (TH1D*)h[6]->Clone();
		numerator_pt_endcap[i] = (TH1D*)h[7]->Clone();
	}
