...
void plot(const TString& category)
{
	...
	//Get ROOT Files
	TFile *f = new TFile("../bin/histograms/Re_fake_TightID_PFIso_EffSF_20181014.root");
...
/*
	// Data-driven same-sign WJets
	TString n_type = "1"; // template fitting
	n_type = "2"; // ratio method
	TH1D *h_WJets_from_data = (TH1D*)f->Get( "fitSameWJets"+n_type+"_Data" );
	TH1D *h_ttbar_SS = (TH1D*)f->Get( "fitSameWJets"+n_type+"_ttbar" );
	TH1D *h_QCD_SS = (TH1D*)f->Get( "fitSameDijet"+n_type+"_Data" );
	h_WJets_from_data->Add(h_QCD_SS,-2.0);
	h_WJets_from_data->Add(h_ttbar_SS,-1.0);
	h_WJets_from_data->SetName("fitSameWJets"+n_type+"_WJets_from_data");

    for(int i=1; i<50; i++)
    {
        if( h_WJets_from_data->GetBinContent(i) < 0 )
        {
            h_WJets_from_data->SetBinContent(i,0.0);
            h_WJets_from_data->SetBinError(i,0.0);
        }
	}
*/
	...
	//TH1D* _h_ = (TH1D*)h_data->Clone();
	TH1D* _h_ = (TH1D*)h_ttbar->Clone();
	//TH1D* _h_ = (TH1D*)h_WJets->Clone();
	//TH1D* _h_ = (TH1D*)h_WJets_from_data->Clone();

	TString hName = _h_->GetName();
	TString charge = " (Opposite sign)";
	if( hName.Contains("Same") ) charge = " (Same sign)";

	MyCanvas *myc = new MyCanvas("../bin/print/wjet_"+hName, "Mass(#mu#mu) [GeV]", "Number of events");
    myc->SetLogx();
	myc->SetLegendPosition(0.6, 0.8, 0.95, 0.85);
	//myc->SetLegendPosition(0.65, 0.8, 0.95, 0.85); //for WJets_from_data
	//myc->CanvasWitOneHistogram( _h_, "Data"+charge, "hist", 11);
	myc->CanvasWitOneHistogram( _h_, "ttbar"+charge, "hist", 4);
	//myc->CanvasWitOneHistogram( _h_, "WJets"+charge, "hist", 9);
    myc->PrintCanvas();
