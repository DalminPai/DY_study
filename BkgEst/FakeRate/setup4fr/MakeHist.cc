...
{
	// -- Setting -- //
	Bool_t is_hist = kFALSE;
	Int_t ntype = 9, ntype2 = 8;
//	Int_t ntype = 8, ntype2 = 7;
	if( rootfile.Contains("hist") )
	{
		is_hist = kTRUE;
		ntype = 21;
		ntype2 = 9;
...
//		h_[8] = (TH1D*)h[12]->Clone(var+"_WJets");
	...
	for(int i=0; i<ntype; i++)
	{
		...
		{
			if( h_[i]->GetName() == (var+"_"+norm[j].first) )
			{
				//cout << h_[i]->GetName() << endl;
