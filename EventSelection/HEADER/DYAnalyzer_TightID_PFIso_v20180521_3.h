...
// -- "EventSelection_ElectronChannel" is eta-restriced version: 18 Dec. 2018 -- //
#pragma once
						...
						vector< Electron >* SelectedElectronCollection) // -- output: 2 electrons passing event selection conditions -- //
{
	...
	for(Int_t j=0; j<(int)ElectronCollection.size(); j++)
	{
		...
		if( elec.passMediumID == kTRUE // modified by Dalmin Pai
			...
			&& elec.Pt > SubPtCut && fabs(elec.etaSC) < SubEtaCut && !( fabs(elec.etaSC) > 1.4442 && fabs(elec.etaSC) < 1.566 ) && fabs(elec.etaSC) > 2.25 )
			QElectronCollection.push_back( ElectronCollection[j] );
	}
						...
						vector< Electron >* SelectedElectronCollection) // -- output: 2 electrons passing event selection conditions -- //
{
	...
	for(Int_t j=0; j<(int)ElectronCollection.size(); j++)
	{
		...
		if( elec.passMediumID == kTRUE // modified by Dalmin Pai
			...
			&& elec.pTUnCorr > SubPtCut && fabs(elec.etaSCUnCorr) > 2.25 && fabs(elec.etaSCUnCorr) < SubEtaCut && !( fabs(elec.etaSCUnCorr) > 1.4442 && fabs(elec.etaSCUnCorr) < 1.566 ) )
			QElectronCollection.push_back( ElectronCollection[j] );
	}
