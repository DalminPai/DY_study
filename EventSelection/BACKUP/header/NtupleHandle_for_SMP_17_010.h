...
//--------------------------------------------------------------------------
// -- 2018.11.20: Copied from "NtupleHandle_v31Oct17.h"
//                Add variables for Medium muon ID
// -- 2018.11.30: Add "HLT_IsoMu24_v* || HLT_IsoTkMu24_v* || HLT_Mu50_v*"
// -- 2018.12.10: Add "HLT_IsoMu22_v* || HLT_IsoTkMu22_v* || HLT_IsoMu24_v* || HLT_IsoTkMu24_v* || HLT_Mu50_v*"
// -- 2018.12.13: Add L1 ECAL prefiring weight
//                Remove "nUnCorrElectron"
// -- 2019.01.24: Add Muon ID flags
/////////////////////////////////////////////////////////////////////////////////////
#pragma once
...
//    Int_t nUnCorrElectron;
    ...
    // -- for Meidum ID --- //
    Double_t Muon_segmentCompatibility[MaxN];
    Double_t Muon_chi2LocalPosition[MaxN];
    Double_t Muon_trkKink[MaxN];
    Double_t Muon_Inner_validFraction[MaxN];
	...
	// for Medium ID
	chain->SetBranchStatus("Muon_segmentCompatibility", 1);
	chain->SetBranchStatus("Muon_chi2LocalPosition", 1);
	chain->SetBranchStatus("Muon_trkKink", 1);
	chain->SetBranchStatus("Muon_Inner_validFraction", 1);
	...
	// for Medium ID
	chain->SetBranchAddress("Muon_segmentCompatibility", &Muon_segmentCompatibility);
	chain->SetBranchAddress("Muon_chi2LocalPosition", &Muon_chi2LocalPosition);
	chain->SetBranchAddress("Muon_trkKink", &Muon_trkKink);
	chain->SetBranchAddress("Muon_Inner_validFraction", &Muon_Inner_validFraction);
    }
	
    // Muon ID flags
    void TurnOnBranches_MuonIDFlags()
    {
...
//    	chain->SetBranchAddress("nUnCorrElectron", &nUnCorrElectron);
    ...
    Bool_t isTriggered(TString HLT)
    {
        ...
        else if( HLT == "HLT_IsoMu24_v* || HLT_IsoTkMu24_v* || HLT_Mu50_v*" )
        {
            for( Int_t k = 0; k < HLT_ntrig; k++ )
            {
                if( (HLT_trigName->at((unsigned int)k)) == "HLT_IsoMu24_v*"
        ...
        else if( HLT == "HLT_IsoMu22_v* || HLT_IsoTkMu22_v* || HLT_IsoMu24_v* || HLT_IsoTkMu24_v* || HLT_Mu50_v*" )
        {
            for( Int_t k = 0; k < HLT_ntrig; k++ )
            {
                if( (HLT_trigName->at((unsigned int)k)) == "HLT_IsoMu24_v*"
			|| (HLT_trigName->at((unsigned int)k)) == "HLT_IsoTkMu24_v*"
			|| (HLT_trigName->at((unsigned int)k)) == "HLT_Mu50_v*"
			|| (HLT_trigName->at((unsigned int)k)) == "HLT_IsoMu22_v*"
			|| (HLT_trigName->at((unsigned int)k)) == "HLT_IsoTkMu22_v*" )
                {
                    if( HLT_trigFired[k] == 1 )
                    {
                        isTrigger = true;
