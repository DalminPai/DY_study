...
// -- Include "ntuple_location.h": 05 Aug. 2018 -- //
// -- Introduce "SetupMCsamples_16Legacy": 06 Aug. 2018 -- //
// -- Add electron efficiency SF of 2016 legacy version: 06 Aug. 2018 -- //
// -- Introduce PVz re-weighting: 03 Sep. 2018 -- //
// -- Add "ZToEE_powheg" in legacy version: 11 Sep. 2018 -- //
#pragma once

#include "Object_v01Dec17.h"
#include "NtupleHandle_v31Oct17.h"
#include "ntuple_location.h"
...
public:
	...
	// -- For efficiency SF of electron (legacy) -- //
	Double_t Eff_Reco_GT20_data_legacy[12][4];
	Double_t Eff_Reco_GT20_MC_legacy[12][4];

	Double_t Eff_Reco_low_data_legacy[10][1];
	Double_t Eff_Reco_low_MC_legacy[10][1];

	Double_t Eff_ID_data_legacy[8][5];
	Double_t Eff_ID_MC_legacy[8][5];
	...
	void SetupMCsamples_16Legacy( TString Type, vector<TString> *ntupleDirectory, vector<TString> *Tag, vector<Double_t> *Xsec, vector<Double_t> *nEvents );
	...
	// legacy
	void SetupEfficiencyScaleFactor_electron_legacy();
	Double_t EfficiencySF_EventWeight_electron_legacy(Electron ele1, Electron ele2);
	Int_t Find_electron_PtBin_Reco_legacy(Double_t Pt);
	Int_t Find_electron_PtBin_ID_legacy(Double_t Pt);
	Int_t Find_electron_EtaBin_Reco_legacy(Double_t eta, Double_t Pt);
	Int_t Find_electron_EtaBin_ID_legacy(Double_t eta);
...
void DYAnalyzer::SetupMCsamples_16Legacy( TString Type, vector<TString> *ntupleDirectory, vector<TString> *Tag, vector<Double_t> *Xsec, vector<Double_t> *nEvents )
{
	if( Type == "DYEE_M10to50" )
	{
		//cout << "# events should be adjusted later" << endl;
		// -- Signal binned samples -- //
		ntupleDirectory->push_back( _DYLL_M10to50_v1_ ); Tag->push_back( "DYEE_M10to50_v1" ); Xsec->push_back( 6016.88 ); nEvents->push_back( 33275218.0 ); //nEvents: sum of DYEE weights
		ntupleDirectory->push_back( _DYLL_M10to50_v2_ ); Tag->push_back( "DYEE_M10to50_v2" ); Xsec->push_back( 6016.88 ); nEvents->push_back( 33275218.0 ); //nEvents: sum of DYEE weights
		ntupleDirectory->push_back( _DYLL_M10to50_ext1v1_ ); Tag->push_back( "DYEE_M10to50_ext1v1" ); Xsec->push_back( 6016.88 ); nEvents->push_back( 33275218.0 ); //nEvents: sum of DYEE weights
	}
	else if( Type == "DYEE_M50toInf" )
	{
		//cout << "# events should be adjusted later" << endl;
		// -- Signal binned samples -- //
		//ntupleDirectory->push_back( _DYLL_M50toInf_ ); Tag->push_back( "DYEE_M50toInf" ); Xsec->push_back( 1952.68432327 ); nEvents->push_back( 27245327.0 ); //nEvents: sum of DYEE weights, NNLO Xsec
		ntupleDirectory->push_back( _DYLL_M50toInf_ ); Tag->push_back( "DYEE_M50toInf" ); Xsec->push_back( 1952.68432327 ); nEvents->push_back( 27199884.0 ); //nEvents: sum of DYEE weights, NNLO Xsec
	}
	else if( Type == "ttbar" )
	{
		//cout << "# events should be adjusted later" << endl;
		// -- Background Samples -- //
		ntupleDirectory->push_back( _ttbar_ ); Tag->push_back( "ttbar" ); Xsec->push_back( 831.76 ); nEvents->push_back( 154948878.0 ); //ttbar + ttbarBackup
	}
	else if( Type == "ttbarBackup" )
	{
		// cout << "# events should be adjusted later" << endl;
		// -- Background Samples -- //
		ntupleDirectory->push_back( _ttbarBackup_ ); Tag->push_back( "ttbarBackup" ); Xsec->push_back( 831.76 ); nEvents->push_back( 154948878.0 ); //ttbar + ttbarBackup
	}
	else if( Type == "DYTauTau_M10to50" )
	{
		// cout << "# events should be adjusted later" << endl;
		// -- Signal binned samples -- //
		ntupleDirectory->push_back( _DYLL_M10to50_v1_ ); Tag->push_back( "DYTauTau_M10to50_v1" ); Xsec->push_back( 6016.88 ); nEvents->push_back( 33080379.0 ); //nEvents: sum of DYTauTau weights
		ntupleDirectory->push_back( _DYLL_M10to50_v2_ ); Tag->push_back( "DYTauTau_M10to50_v2" ); Xsec->push_back( 6016.88 ); nEvents->push_back( 33080379.0 ); //nEvents: sum of DYTauTau weights
		ntupleDirectory->push_back( _DYLL_M10to50_ext1v1_ ); Tag->push_back( "DYTauTau_M10to50_ext1v1" ); Xsec->push_back( 6016.88 ); nEvents->push_back( 33080379.0 ); //nEvents: sum of DYTauTau weights
	}
	else if( Type == "DYTauTau_M50toInf" )
	{
		//cout << "# events should be adjusted later" << endl;
		// -- Signal binned samples -- //
		//ntupleDirectory->push_back( _DYLL_M50toInf_ ); Tag->push_back( "DYTauTau" ); Xsec->push_back( 1952.68432327 ); nEvents->push_back( 27277866.0 ); //nEvents: sum of DYTauTau weights, NNLO Xsec
		ntupleDirectory->push_back( _DYLL_M50toInf_ ); Tag->push_back( "DYTauTau" ); Xsec->push_back( 1952.68432327 ); nEvents->push_back( 27232310.0 ); //nEvents: sum of DYTauTau weights, NNLO Xsec
	}
	else if( Type == "VVnST" )
	{
		// cout << "# events should be adjusted later" << endl;
		// -- Background Samples -- //
		ntupleDirectory->push_back( _ST_tW_ ); Tag->push_back( "tW" ); Xsec->push_back( 35.85 ); nEvents->push_back( 6952830.0 );
		ntupleDirectory->push_back( _ST_tbarW_ ); Tag->push_back( "tbarW" ); Xsec->push_back( 35.85 ); nEvents->push_back( 6933093.0 );
		ntupleDirectory->push_back( _ZZ_ ); Tag->push_back( "ZZ" ); Xsec->push_back( 16.523 ); nEvents->push_back( 998034.0 );
		ntupleDirectory->push_back( _WZ_ ); Tag->push_back( "WZ" ); Xsec->push_back( 47.13 ); nEvents->push_back( 2995828.0 );
		ntupleDirectory->push_back( _WW_ ); Tag->push_back( "WW" ); Xsec->push_back( 118.7 ); nEvents->push_back( 6987123.0 );
		//ntupleDirectory->push_back( _WWTo2L2Nu_ ); Tag->push_back( "WW" ); Xsec->push_back( 12.178 ); nEvents->push_back( 1999000.0 );
	}
	else if( Type == "WJetsToLNu" )
	{
		//cout << "# events should be adjusted later" << endl;
		// -- Background Samples -- //
		//ntupleDirectory->push_back( _WJetsToLNu_ ); Tag->push_back( "WJetsToLNu" ); Xsec->push_back( 61526.7 ); nEvents->push_back( 86731698.0 );
		//ntupleDirectory->push_back( _WJetsToLNu_ext_ ); Tag->push_back( "WJetsToLNu_ext" ); Xsec->push_back( 61526.7 ); nEvents->push_back( 86731698.0 );
		ntupleDirectory->push_back( _WJetsToLNu_ ); Tag->push_back( "WJetsToLNu" ); Xsec->push_back( 61526.7 ); nEvents->push_back( 86465390.0 );
		ntupleDirectory->push_back( _WJetsToLNu_ext_ ); Tag->push_back( "WJetsToLNu_ext" ); Xsec->push_back( 61526.7 ); nEvents->push_back( 86465390.0 );
	}
	else if( Type == "ZToEE_powheg" )
	{
		//cout << "# events should be adjusted later" << endl;
		// -- Signal binned samples -- //
		//NLO Xsec from AN2016_404_v8
		ntupleDirectory->push_back( _ZToEE_M50to120_ ); Tag->push_back( "ZToEE_M50to120" ); Xsec->push_back(1975.0); nEvents->push_back(2998396.0);
		ntupleDirectory->push_back( _ZToEE_M120to200_ ); Tag->push_back( "ZToEE_M120to200" ); Xsec->push_back(19.32); nEvents->push_back(100000.0);
		ntupleDirectory->push_back( _ZToEE_M200to400_ ); Tag->push_back( "ZToEE_M200to400" ); Xsec->push_back(2.73); nEvents->push_back(99200.0);
		ntupleDirectory->push_back( _ZToEE_M400to800_ ); Tag->push_back( "ZToEE_M400to800" ); Xsec->push_back(0.241); nEvents->push_back(100000.0);
		ntupleDirectory->push_back( _ZToEE_M800to1400_ ); Tag->push_back( "ZToEE_M800to1400" ); Xsec->push_back(1.68E-2); nEvents->push_back(100000.0);
		ntupleDirectory->push_back( _ZToEE_M1400to2300_ ); Tag->push_back( "ZToEE_M1400to2300" ); Xsec->push_back(1.39E-3); nEvents->push_back(100000.0);
		ntupleDirectory->push_back( _ZToEE_M2300to3500_ ); Tag->push_back( "ZToEE_M2300to3500" ); Xsec->push_back(8.948E-5); nEvents->push_back(100000.0);
		ntupleDirectory->push_back( _ZToEE_M3500to4500_ ); Tag->push_back( "ZToEE_M3500to4500" ); Xsec->push_back(4.135E-6); nEvents->push_back(100000.0);
		ntupleDirectory->push_back( _ZToEE_M4500to6000_ ); Tag->push_back( "ZToEE_M4500to6000" ); Xsec->push_back(4.56E-7); nEvents->push_back(99000.0);
		ntupleDirectory->push_back( _ZToEE_M6000toInf_ ); Tag->push_back( "ZToEE_M6000toInf" ); Xsec->push_back(2.06E-8); nEvents->push_back(99800.0);
	}
	else
		cout << "Wrong Type!" << endl;
}
...
void DYAnalyzer::SetupEfficiencyScaleFactor_electron_legacy()
{
	TString Location = "../../TOOL/effSF/effSF_electron/";
	...
	// -- Reco SF -- //
	///////////////////
	TFile *f1_low = new TFile( Location+"/legacy/EGM2D_BtoH_low_RecoSF_Legacy2016.root" );
	TGraphErrors *h_reco_sf_low = (TGraphErrors*)f1_low->Get("grSF1D_0");

	Int_t nEtaBins_reco_low = h_reco_sf_low->GetN(); //10
	Int_t nPtBins_reco_low = 1;

	Double_t x_reco_low[10]; Double_t xx_reco_low[10];
	Double_t y_reco_low[10]; Double_t yy_reco_low[10];

	for(Int_t iter_y = 0; iter_y < nPtBins_reco_low; iter_y++)
	{
		for(Int_t i=0; i<nEtaBins_reco_low; i++)
		{
			h_reco_sf_low->GetPoint(i, x_reco_low[i], y_reco_low[i]);
		}

		// -- Rearrangement in order of x values -- //
		Double_t xmin;
		Double_t xlow = -9999;
		for(Int_t j=0; j<nEtaBins_reco_low; j++)
		{
			Int_t jj = -9999;

			xmin = 9999;
			for(Int_t k=0; k<nEtaBins_reco_low; k++)
			{
				if( xlow < x_reco_low[k] && x_reco_low[k] < xmin )
				{
					jj = k;
					xmin = x_reco_low[k];
				}
			}
			xx_reco_low[j] = x_reco_low[jj];
			yy_reco_low[j] = y_reco_low[jj];

			xlow = xmin;
//      cout << j << "  " << xx_reco[j] << "  " << yy_reco[j] << endl;
		} // End of rearrangement

		for(Int_t iter_x = 0; iter_x < nEtaBins_reco_low; iter_x++)
		{
			Eff_Reco_low_data_legacy[iter_x][iter_y] = yy_reco_low[iter_x]; // actually, it is the scale factor.
			Eff_Reco_low_MC_legacy[iter_x][iter_y] = 1;
//      cout << "Reco: bin# = [" << iter_x << ", " << iter_y << "]" << " eta = " << xx_reco[iter_x] << " sf = " << yy_reco[iter_x] << endl;
		}
	}

	// -- Reco SF for pT > 20 GeV -- //
	TFile *f1_GT20 = new TFile( Location+"/legacy/EGM2D_BtoH_GT20GeV_RecoSF_Legacy2016.root" );
	TGraphErrors *h_reco_sf_0 = (TGraphErrors*)f1_GT20->Get("grSF1D_0");
	TGraphErrors *h_reco_sf_1 = (TGraphErrors*)f1_GT20->Get("grSF1D_1");
	TGraphErrors *h_reco_sf_2 = (TGraphErrors*)f1_GT20->Get("grSF1D_2");
	TGraphErrors *h_reco_sf_3 = (TGraphErrors*)f1_GT20->Get("grSF1D_3");

	Int_t nEtaBins_reco = h_reco_sf_0->GetN(); //12
	Int_t nPtBins_reco = 4;

	Double_t x_reco[12]; Double_t xx_reco[12];
	Double_t y_reco[12]; Double_t yy_reco[12];

	TGraphErrors *h_reco_sf;
	for(Int_t iter_y = 0; iter_y < nPtBins_reco; iter_y++)
	{
		if(iter_y == 0) h_reco_sf = (TGraphErrors*)h_reco_sf_0->Clone();
		else if(iter_y == 1) h_reco_sf = (TGraphErrors*)h_reco_sf_1->Clone();
		else if(iter_y == 2) h_reco_sf = (TGraphErrors*)h_reco_sf_2->Clone();
		else if(iter_y == 3) h_reco_sf = (TGraphErrors*)h_reco_sf_3->Clone();
		...
		for(Int_t iter_x = 0; iter_x < nEtaBins_reco; iter_x++)
		{
			Eff_Reco_GT20_data_legacy[iter_x][iter_y] = yy_reco[iter_x]; // actually, it is the scale factor.
			Eff_Reco_GT20_MC_legacy[iter_x][iter_y] = 1;
			//cout << "ID: bin# = [" << iter_x << ", " << iter_y << "]" << " eta = " << xx_id[iter_x] << " sf = " << yy_id[iter_x] << endl;
		}
	}
	/////////////////
	// -- ID SF -- //
	/////////////////
	TFile *f2 = new TFile( Location+"/legacy/2016LegacyReReco_ElectronMedium.root" );
	TGraphErrors *h_id_sf_0 = (TGraphErrors*)f2->Get("grSF1D_0");
	TGraphErrors *h_id_sf_1 = (TGraphErrors*)f2->Get("grSF1D_1");
	TGraphErrors *h_id_sf_2 = (TGraphErrors*)f2->Get("grSF1D_2");
	TGraphErrors *h_id_sf_3 = (TGraphErrors*)f2->Get("grSF1D_3");
	...
	Int_t nEtaBins_id = h_id_sf_0->GetN(); //8
	Int_t nPtBins_id = 5;

	Double_t x_id[8]; Double_t xx_id[8];
	Double_t y_id[8]; Double_t yy_id[8];
	...
	for(Int_t iter_y = 0; iter_y < nPtBins_id; iter_y++)
	{
		...
		for(Int_t iter_x = 0; iter_x < nEtaBins_id; iter_x++)
		{
			Eff_ID_data_legacy[iter_x][iter_y] = yy_id[iter_x]; // actually, it is the scale factor.
			Eff_ID_MC_legacy[iter_x][iter_y] = 1;
			//cout << "ID: bin# = [" << iter_x << ", " << iter_y << "]" << " eta = " << xx_id[iter_x] << " sf = " << yy_id[iter_x] << endl;
		}
	}
	/////////////////////////////
...
Double_t DYAnalyzer::EfficiencySF_EventWeight_electron_legacy(Electron ele1, Electron ele2)
{
	Double_t weight = -999;
	...
	Int_t ptbin1_Reco = Find_electron_PtBin_Reco_legacy( Pt1 );
	Int_t etabin1_Reco = Find_electron_EtaBin_Reco_legacy( eta1, Pt1 );

	Int_t ptbin1_ID = Find_electron_PtBin_ID_legacy( Pt1 );
	Int_t etabin1_ID = Find_electron_EtaBin_ID_legacy( eta1 );
	//cout << "Pt1: " << Pt1 << "\teta1: " << eta1 << endl;
	//cout << "ptbin1_ID: " << ptbin1_ID << "\tetabin1_ID: " << etabin1_ID << endl;
	//cout << "Eff_SF: " << Eff_ID_data[etabin1_ID][ptbin1_ID] << endl;
	...
	Double_t Eff_ele1_data = 0, Eff_ele1_MC = 0;
	if( Pt1 < 20 )
	{
		Eff_ele1_data = Eff_Reco_low_data_legacy[etabin1_Reco][ptbin1_Reco] * Eff_ID_data_legacy[etabin1_ID][ptbin1_ID];
		Eff_ele1_MC = Eff_Reco_low_MC_legacy[etabin1_Reco][ptbin1_Reco] * Eff_ID_MC_legacy[etabin1_ID][ptbin1_ID];
	}
	else
	{
		Eff_ele1_data = Eff_Reco_GT20_data_legacy[etabin1_Reco][ptbin1_Reco] * Eff_ID_data_legacy[etabin1_ID][ptbin1_ID];
		Eff_ele1_MC = Eff_Reco_GT20_MC_legacy[etabin1_Reco][ptbin1_Reco] * Eff_ID_MC_legacy[etabin1_ID][ptbin1_ID];
	}
	...
	Int_t ptbin2_Reco = Find_electron_PtBin_Reco_legacy( Pt2 );
	Int_t etabin2_Reco = Find_electron_EtaBin_Reco_legacy( eta2, Pt2 );

	Int_t ptbin2_ID = Find_electron_PtBin_ID_legacy( Pt2 );
	Int_t etabin2_ID = Find_electron_EtaBin_ID_legacy( eta2 );

	Int_t ptbin2_Trig = Find_electron_PtBin_Trig( Pt2 );
	Int_t etabin2_Trig = Find_electron_EtaBin_Trig( eta2 );

	Double_t Eff_ele2_data = 0, Eff_ele2_MC = 0;
	if( Pt2 < 20 )
	{
		Eff_ele2_data = Eff_Reco_low_data_legacy[etabin2_Reco][ptbin2_Reco] * Eff_ID_data_legacy[etabin2_ID][ptbin2_ID];
		Eff_ele2_MC = Eff_Reco_low_MC_legacy[etabin2_Reco][ptbin2_Reco] * Eff_ID_MC_legacy[etabin2_ID][ptbin2_ID];
	}
	else
	{
		Eff_ele2_data = Eff_Reco_GT20_data_legacy[etabin2_Reco][ptbin2_Reco] * Eff_ID_data_legacy[etabin2_ID][ptbin2_ID];
		Eff_ele2_MC = Eff_Reco_GT20_MC_legacy[etabin2_Reco][ptbin2_Reco] * Eff_ID_MC_legacy[etabin2_ID][ptbin2_ID];
...
Int_t DYAnalyzer::Find_electron_PtBin_Reco_legacy(Double_t Pt)
{
	if( Pt < 20 ) return 0;
	else
	{
		const Int_t nPtBins = 4;
		Double_t PtBinEdges[nPtBins+1] = {20, 45, 75, 100, 500};
...
Int_t DYAnalyzer::Find_electron_PtBin_ID_legacy(Double_t Pt)
{
	const Int_t nPtBins = 5;
	Double_t PtBinEdges[nPtBins+1] = {20, 35, 50, 90, 150, 500};
...
Int_t DYAnalyzer::Find_electron_EtaBin_Reco_legacy(Double_t eta, Double_t Pt)
{
	if( Pt < 20 )
	{
		const Int_t nEtaBins = 10;
		Double_t EtaBinEdges[nEtaBins+1] = {-2.5, -2.0, -1.566, -1.4442, -1.0, 0.0, 1.0, 1.4442, 1.566, 2.0, 2.5};
	...
	else
	{
		...
		Double_t EtaBinEdges[nEtaBins+1] = {-2.5, -2.0, -1.566, -1.4442, -1.0, -0.5, 0.0, 0.5, 1.0, 1.4442, 1.566, 2.0, 2.5};
...
Int_t DYAnalyzer::Find_electron_EtaBin_ID_legacy(Double_t eta)
{
	const Int_t nEtaBins = 10;
	Double_t EtaBinEdges[nEtaBins+1] = {-2.5, -2.0, -1.566, -1.4442, -0.8, 0.0, 0.8, 1.4442, 1.566, 2.0, 2.5};
