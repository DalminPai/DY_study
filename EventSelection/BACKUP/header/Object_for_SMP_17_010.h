...
//--------------------------------------------------------------------------
// -- 2018.11.20: Copied from "Object_v01Dec17.h"
//                Add Medium muon ID with it's variables
...
public:
	...
	// for Medium ID
	Double_t segmentCompatibility;
	Double_t chi2LocalPosition;
	Double_t trkKink;
	Double_t Inner_validFraction;
	...
	void FillFromNtuple(NtupleHandle *ntuple, Int_t index)
	{
		...
		// for Medium ID
		segmentCompatibility = ntuple->Muon_segmentCompatibility[index];
...
//			&& this->trackerLayers > 5
	...
	// -- https://twiki.cern.ch/twiki/bin/viewauth/CMS/SWGuideMuonIdRun2#Medium%20Muon -- //
	Bool_t isMediumMuon()
	{
		if( this->isPF == 1
			&& ( this->isGLB == 1 || this->isTRK == 1 )
			&& this->Inner_validFraction > 0.8
			&& ( this->segmentCompatibility > 0.451 || ( this->isGLB == 1
									&& this->chi2dof < 3
									&& this->chi2LocalPosition < 12
									&& this->trkKink < 20
									&& this->segmentCompatibility > 0.303 ) ) )
		{
			return 1;
		}
