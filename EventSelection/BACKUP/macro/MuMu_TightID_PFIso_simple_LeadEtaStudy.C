...
void MuMu_TightID_PFIso_simple_LeadEtaStudy(Int_t debug, Int_t type, Int_t remainder = 9999, Int_t isTopPtReweighting = 0, TString HLTname = "IsoMu24_OR_IsoTkMu24")
{
		...
		{	
			...
			if( isMC == kTRUE && GenFlag == kTRUE && GenFlag_top == kTRUE )
			{
				...
				if( GenLeptonCollection.size() == 2 ) // -- Select the events containing 2 muons from hard-process -- //
				{
					...
					if( GenLeptonCollection[0].Pt > GenLeptonCollection[1] )
					{
					...
					if( isPassAcc_GenLepton == kTRUE )
					{
						...
						for(Int_t i=0; i<6; i++)
						{
							Int_t pt_min = (i+3) * 10;
							Int_t pt_max = (i+4) * 10;
							if( pt_min < genlep1.Pt && genlep1.Pt < pt_max ) h_Gen_pt_leadEta[i]->Fill( genlep1.eta, TotWeight );
							if( pt_min < genlep2.Pt && genlep2.Pt < pt_max ) h_Gen_pt_subEta[i]->Fill( genlep2.eta, TotWeight );
						}
					} //isPassAcc_GenLepton
			...
			if( TriggerFlag == kTRUE && GenFlag == kTRUE && GenFlag_top == kTRUE )
			{
				...
				if( isPassEventSelection == kTRUE )
				{
					...
					for(Int_t i=0; i<6; i++)
					{
						Int_t pt_min = (i+3) * 10;
						Int_t pt_max = (i+4) * 10;
						if( pt_min < mu1.Pt && mu1.Pt < pt_max ) h_pt_leadEta[i]->Fill( mu1.eta, TotWeight * PUWeight * effweight );
						if( pt_min < mu2.Pt && mu2.Pt < pt_max ) h_pt_subEta[i]->Fill( mu2.eta, TotWeight* PUWeight * effweight );
					}
