...
#include </scratch/DYanalysis/RoccoR/RoccoR.cc>
...
void MuMu_TightID_PFIso_BtoF_v20180521(Int_t type, Int_t Num = 100, Int_t isTopPtReweighting = 0, TString HLTname = "IsoMu24_OR_IsoTkMu24")
{
	// -- Run2016 luminosity [/pb] -- //
	Double_t L = Lumi_BtoF;
	...
	Double_t D = 1;
	if( type == 1 || type == 6 || type == 7 || type == 11 || type == 31 || type == 51 ) D = 2;
	...
	RoccoR rc(DY_Tools+"/RoccoR/rcdata.2016.v3");

	// -- Efficiency SF setup -- //
	analyzer->SetupEfficiencyScaleFactor_BtoF();

	// -- Output ROOTFile -- //	
	TFile *f = new TFile(OutputDir+"/dimuon/ROOTFile_MuMu_TightID_PFIso_BtoF_v20180521_"+TString::Itoa(type,10)+"_"+TString::Itoa(Num,10)+"_"
	...
	for(Int_t i_tup = 0; i_tup<Ntup; i_tup++)
	{
		...
		{
			if( Num == 100 )
				chain->Add(BaseLocation+"/v2.1/"+ntupleDirectory[i_tup]+"/*.root");
			else
				for(Double_t ii=1; ii<=SIZE[type]; ii++)
					if(ii - TMath::Floor(ii/D) * D == Num)
						chain->Add(BaseLocation+"/v2.1/"+ntupleDirectory[i_tup]+"/*_"+TString::Itoa(ii,10)+".root");
		}
		//Set Data chain
		else
		{
			if(Num==100)
			{
				chain->Add(BaseLocation+"/v2.0/"+DataLocation+"/*.root");
				if(type==7) chain->Add(BaseLocation+"/v2.0/SingleMuon_Run2016Hver3/*.root");
			}
			else
				for(Double_t ii=1; ii<=SIZE[type]; ii++)
					if(ii - TMath::Floor(ii/D) * D == Num)
					{
						chain->Add(BaseLocation+"/v2.0/"+DataLocation+"/*_"+TString::Itoa(ii,10)+".root");
						if(type==7 && ii<=31) chain->Add(BaseLocation+"/v2.0/SingleMuon_Run2016Hver3/*_"+TString::Itoa(ii,10)+".root");
					}
		}
		...
		{	
			...
			if( TriggerFlag == kTRUE && GenFlag == kTRUE && GenFlag_top == kTRUE )
			{
				...
				if( isPassEventSelection == kTRUE )
				{
					...
					if( isMC == kTRUE )
					{
						effweight = analyzer->EfficiencySF_EventWeight_HLT_BtoF( mu1, mu2 );
						effweight_Reco = analyzer->EfficiencySF_EventWeight_HLT_BtoF_Reco( mu1, mu2 );
						effweight_RecoID = analyzer->EfficiencySF_EventWeight_HLT_BtoF_RecoID( mu1, mu2 );
						effweight_RecoIDIso = analyzer->EfficiencySF_EventWeight_HLT_BtoF_RecoIDIso( mu1, mu2 );

						if( 60 < reco_M && reco_M < 120 )
							SF_LeadEtaCorr = analyzer->LeadEtaCorr( type, "BtoF", mu1, mu2 );
					}
