...
void MuMu_TightID_PFIso_GtoH_v20180521(Int_t type, Int_t Num = 100, Int_t isTopPtReweighting = 0, TString HLTname = "IsoMu24_OR_IsoTkMu24")
{
	// -- Run2016 luminosity [/pb] -- //
	Double_t L = Lumi_GtoH;
	...
	// -- Output ROOTFile -- //	
	TFile *f = new TFile(OutputDir+"/dimuon/ROOTFile_MuMu_TightID_PFIso_GtoH_v20180521_"+TString::Itoa(type,10)+"_"+TString::Itoa(Num,10)+"_"
		...
		{	
			...
			if( TriggerFlag == kTRUE && GenFlag == kTRUE && GenFlag_top == kTRUE )
			{
				...
				if( isPassEventSelection == kTRUE )
				{
					...
					if( isMC == kTRUE )
					{
						effweight = analyzer->EfficiencySF_EventWeight_HLT_GtoH( mu1, mu2 );
						effweight_Reco = analyzer->EfficiencySF_EventWeight_HLT_GtoH_Reco( mu1, mu2 );
						effweight_RecoID = analyzer->EfficiencySF_EventWeight_HLT_GtoH_RecoID( mu1, mu2 );
						effweight_RecoIDIso = analyzer->EfficiencySF_EventWeight_HLT_GtoH_RecoIDIso( mu1, mu2 );

						if( 60 < reco_M && reco_M < 120 )
							SF_LeadEtaCorr = analyzer->LeadEtaCorr( type, "GtoH", mu1, mu2 );
					}
