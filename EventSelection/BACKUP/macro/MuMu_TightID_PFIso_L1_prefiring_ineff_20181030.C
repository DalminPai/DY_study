...
// -- Add L1 pre-firing inefficiency : 30 Oct. 2018 -- //
void MuMu_TightID_PFIso_L1_prefiring_ineff_20181030(Int_t debug, Int_t type, Int_t remainder = 9999, Int_t isTopPtReweighting = 0, TString HLTname = "IsoMu24_OR_IsoTkMu24")
{
	...
	// -- Output ROOTFile -- //	
	TString Output_ROOTFile = BaseDir+"/RESULT/MuMu/ROOTFile_20181030_MuMu_TightID_PFIso_L1_prefiring_ineff_"+TString::Itoa(type,10)+"_"+TString::Itoa(remainder,10)+"_"
	...
	for(Int_t i_tup = 0; i_tup<Ntup; i_tup++)
	{
		...
		{	
			...
			Double_t l1_prefiring_ineff = analyzer->L1_prefiring_ineff( type, ntuple );
			TotWeight *= l1_prefiring_ineff;
			...
			if( TriggerFlag == kTRUE && GenFlag == kTRUE && GenFlag_top == kTRUE )
			{
				...
				if( isPassEventSelection == kTRUE )
				{
					...
					{
						if( abs(mu1.eta) > 2.0 ) 
							cout << "(Njet, L1 pre-firing ineff) = (" << ntuple->Njets << ", " << l1_prefiring_ineff << ")" << endl;
