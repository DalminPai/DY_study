...
void MuMu_IDtest_for_SMP_17_010(Int_t debug, Int_t type, Int_t remainder = 9999, Int_t isTopPtReweighting = 0, TString HLTname = "IsoMu22_OR_IsoTkMu22_OR_IsoMu24_OR_IsoTkMu24_OR_Mu50_for_SMP_17_010")
{
	...
	for(Int_t i_tup = 0; i_tup<Ntup; i_tup++)
	{
		...
		Int_t dd=0, ee=0, ff=0, gg=0; // check for Muon ID flags
		...
		{	
			...
			if( TriggerFlag == kTRUE && GenFlag == kTRUE && GenFlag_top == kTRUE )
			{
				...
				for(Int_t i_reco=0; i_reco<NLeptons; i_reco++)
				{
					...
					// -- ID test ----------------------------------------------------------------------
					if( mu.isMediumMuon() != mu.passMediumID )
					{
						cout << "Different: Muon Medium ID" << endl;
						cout <<"\tMedium(var, flag) = (" << mu.isMediumMuon() << ", " << mu.passMediumID << ")" << endl;
						dd += 1;
					}
					else
					{
						//cout << "Same: Muon Medium ID" << endl;
						ee += 1;
					}

					if( mu.isLooseMuon() != mu.passLooseID )
					{
						cout << "Different: Muon Loose ID" << endl;
						cout <<"\tLoose(var, flag) = (" << mu.isLooseMuon() << ", " << mu.passLooseID << ")" << endl;
						ff += 1;
					}
					else
					{
						//cout << "Same: Muon Loose ID" << endl;
						gg += 1;
					}
					// ---------------------------------------------------------------------------------
		...
		cout << "Medium (diff, same) = (" << dd << ", " << ee << ")" << endl; // check for Muon Medium flag
		cout << "Loose (diff, same) = (" << ff << ", " << gg << ")" << endl; // check for Muon Loose flag
