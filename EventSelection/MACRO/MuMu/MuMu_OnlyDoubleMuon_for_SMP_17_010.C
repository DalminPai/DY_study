...
// -- 2019.01.22: Use "SetupOnlyDoubleMuon" and "isOnlyDoubleMuon"
//
//
...
void MuMu_OnlyDoubleMuon_for_SMP_17_010(Int_t debug, Int_t type, Int_t remainder = 9999, Int_t isTopPtReweighting = 0, TString HLTname = "IsoMu22_OR_IsoTkMu22_OR_IsoMu24_OR_IsoTkMu24_OR_Mu50_for_SMP_17_010")
{
	...
	// -- OnlyDoubleMuon setup -- //
	analyzer->SetupOnlyDoubleMuon(type);
	...
	TString Output_ROOTFile = BaseDir+"/RESULT/MuMu/ROOTFile_20190122_MuMu_OnlyDoubleMuon_for_SMP_17_010_"+DataType
	...
	for(Int_t i_tup = 0; i_tup<Ntup; i_tup++)
	{
		...
		{	
			...
			// -- Check if it is OnlyDoubleMuon event -----------------------------------------------
			Bool_t isOnlyDoubleMuonFlag = kFALSE;
			if( isMC == kFALSE ) isOnlyDoubleMuonFlag = analyzer->isOnlyDoubleMuon( ntuple->evtNum );
			if( isOnlyDoubleMuonFlag == kFALSE ) continue;
			// --------------------------------------------------------------------------------------
