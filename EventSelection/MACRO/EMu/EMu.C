...
void EMu(Int_t debug, Int_t type, Int_t remainder = 9999, Int_t isTopPtReweighting = 0, TString HLTname = "IsoMu24_OR_IsoTkMu24")
{
	...
	for(Int_t i_tup = 0; i_tup<Ntup; i_tup++)
	{
		...
		TH1D *h_ele_pT = (TH1D*)h_mu_pT->Clone("h_ele_pT_"+Tag[i_tup]);
		TH1D *h_ele_eta = (TH1D*)h_mu_eta->Clone("h_ele_eta_"+Tag[i_tup]);
		TH1D *h_ele_etaSC = (TH1D*)h_mu_eta->Clone("h_ele_etaSC_"+Tag[i_tup]);
		TH1D *h_ele_phi = (TH1D*)h_mu_phi->Clone("h_ele_phi_"+Tag[i_tup]);

		TH1D *h_emuSS_mass = (TH1D*)h_emu_mass->Clone("h_emuSS_mass_"+Tag[i_tup]);
		TH1D *h_emuSS_mass_fine = (TH1D*)h_emu_mass_fine->Clone("h_emuSS_mass_fine_"+Tag[i_tup]);
		TH1D *h_muSS_pT = (TH1D*)h_mu_pT->Clone("h_muSS_pT_"+Tag[i_tup]);
		TH1D *h_muSS_eta = (TH1D*)h_mu_eta->Clone("h_muSS_eta_"+Tag[i_tup]);
		TH1D *h_muSS_phi = (TH1D*)h_mu_phi->Clone("h_muSS_phi_"+Tag[i_tup]);
		TH1D *h_eleSS_pT = (TH1D*)h_mu_pT->Clone("h_eleSS_pT_"+Tag[i_tup]);
		TH1D *h_eleSS_eta = (TH1D*)h_mu_eta->Clone("h_eleSS_eta_"+Tag[i_tup]);
